...
while True:
    ...
    for member in member_data_decoded["data"]:
        if (
            len(member["relationships"]["currently_entitled_tiers"]["data"])
            > 0
            and member["relationships"]["currently_entitled_tiers"]["data"][0][
                "id"
            ]
            == "3952333"
        ):
...
with open("patreon.h", "w", encoding="utf-8") as text_file:
